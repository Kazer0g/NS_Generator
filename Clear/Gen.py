...
class Main_Window (tk.Tk):
	...
	# ----------------------------------------Смена сторон)
	def swithch (self, equal):
		equal.left_cc, equal.right_cc = equal.right_cc, equal.left_cc
		equal.left_full, equal.right_full = equal.right_full, equal.left_full
		equal.left_mark, equal.right_mark = equal.right_mark, equal.left_mark
	...
	def randNum (self, equal):
		...
		if len(equal.left_full) < len(equal.right_full):
			self.swithch (equal)
	# ------------------------------Закрашивание числа)
	...
	def check_valans (self, equal, left, right):
		...
		for i in range(len(right)):
			if (len(correct_right) != len(right)) or ((right[i] != correct_right[i]) and (right[i] != '*')):
				return False
		return True
	
	#---------------------------- Все варианты решения)
	def all_answers (self, equal):
		count = counter (equal.left_mark, '*')
	...
	def cut_variants (self, equal):
		...
		while (len(equal.list_answers)>1) or (equal.left_mark.find('*') == -1) or (equal.right_mark.find('*') == -1):
			index = equal.left_mark.find('*')
			index_sec = equal.right_mark.find('*')
			if (index != -1) and (index_sec != -1):
				...
				if counter(equal.left_mark, '*') < (counter(equal.right_mark, '*')):
					self.swithch(equal)
	...
	def write (self):
		...
		for i in range(len(self.list_Equals)):
			...
			if self.mod == 1:
				...
				par = answers.add_paragraph(str(i+1) + ') ')
				for j in range(len(equal.list_answers)):
					left_full = par.add_run(equal.list_answers[j])
					left_cc = par.add_run (str(equal.left_cc))
					left_cc.font.subscript = True
					eq = par.add_run ('=')
					right_full = par.add_run (int_to_NS(NS_to_int(equal.list_answers[j], equal.left_cc), equal.right_cc))
					right_cc = par.add_run (str(equal.right_cc))
					right_cc.font.subscript = True
					n = par.add_run ('\n')
			# ------------------------------Запись заданий)
			...
			left_cc.font.subscript = True
			eq = par.add_run ('=')
			right_mark = par.add_run (equal.right_mark)
			right_cc = par.add_run (str(equal.right_cc))
			right_cc.font.subscript = True
